...
(NAME = Tfs_<collection>_data,  
    FILENAME = 'Z:\SQLData\Tfs_<collection>.mdf',  
    SIZE = 10,  
    MAXSIZE = 50,  
    FILEGROWTH = 5)
LOG ON
(NAME = Tfs_<collection>_log,  
    FILENAME = 'Z:\SQLData\Tfs_<collection>.ldf',  
    SIZE = 5MB,  
    MAXSIZE = 25MB,  
    FILEGROWTH = 5MB);
GO  
...
RESTORE FILELISTONLY FROM DISK = 'Z:\TFS\<backup file name>' 

-- Restore backup (change the script to use the correct files)

RESTORE DATABASE Tfs_<collection> FROM DISK = 'Z:\TFS\<backup file name>' WITH NORECOVERY, 
      MOVE 'Tfs_<collection>' TO 'Z:\SQLData\Tfs_<collection>.mdf',
      MOVE 'Tfs_<collection>_log' TO 'Z:\SQLData\Tfs_<collection>.ldf',
      REPLACE
GO
